...
public abstract class AgHandlerBESA implements Serializable {
     ...
     * @throws SystemDirectoryExceptionBESA 
     */
    /*public synchronized void notifyCheckpoint() throws SystemDirectoryExceptionBESA {
        try {
            this.notify();
        } catch (Exception e) {
            ReportBESA.error("Happened an error on the notify all of checkpoint agent: " + e.toString());
            throw new SystemDirectoryExceptionBESA("Happened an error on the notify all checkpoint agent: " + e.toString());
        }
    }*/
